...
class Trainer:
    ...
    def embed_tokens(self, tokenized_text_file, tokens_file, context_window):
        ...
        r.close()
        t.close()
        #embedding non-normalized values
        embedded_token_vectors = {}
        for i in tokens:
            embedded_token_vectors[i] = [0 for i in tokens]
        for i in range(len(tokenized_text)):
            for j in range(context_window):
                k=j+1
                if i-j>=0:
                    embedded_token_vectors[tokenized_text[i]][tokens.index(tokenized_text[i-k])] += 1/k
            print((str(round((i*100)/len(tokenized_text),2)))+"% complete with embedding")
        ...
        for i in embedded_token_vectors.keys(): #normalization
            ...
            for j in embedded_token_vectors[i]:
                if not highest_value == 0:
                    normalized_values.append(round(j/highest_value,5))
                else:
